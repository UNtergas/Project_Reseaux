...
def SceneMultiJoin(self):
    ...
    self.box["inputbox"] = InputBox(
        self.game.screen_width/2, self.game.screen_height/2-90, 600, 45, lambda: None, font2)

    self.box["namebox"] = InputBox(
        self.game.screen_width/2, self.game.screen_height/2, 600, 45, lambda: None, font2)

    self.buttons['button_menu'] = Button_text(self.game.screen_width/2-150, self.game.screen_height/2+50,
                                              300, 100, lambda: self.game.switchScene(SCENE_MENU_ID), "Back to Menu")

    self.buttons['join_button'] = Button_text(self.game.screen_width/2+150, self.game.screen_height/2+50,
                                              300, 100, lambda: pygame.event.post(pygame.event.Event(
                                                  event_types["LaunchGame"], {"room_name": self.box["inputbox"].text, "player_name": self.box["namebox"].text})), "Join your room")
...
def SceneMultiJoinRun(self):
    ...
    pygame.draw.rect(self.game.screen, (180, 180, 180),
                     (self.game.screen_width/2-300, self.game.screen_height/2-150, 600, 275))
