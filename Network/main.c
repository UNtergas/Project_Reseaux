...
void *bindNewDiscoveringThread(void *arg) {
    ...
    while (1) {
        if (recvfrom(hostSocket, buffer, 63, 0, (struct sockaddr *)&hostAddr, (socklen_t *)&host_len) >= 0) {
            ...
            write(fd, "Successfully receive request", strlen("Successfully receive request"));
            write(fd, "\n", 1);
            close(fd);
        } else {
            int fd = open("/home/john/Desktop/demo.log", O_RDWR | O_APPEND | O_CREAT);
            write(fd, "Failed receive request", strlen("Failed receive request"));
            write(fd, "\n", 1);
            close(fd);
        }
...
int main(int argc, char **argv) {
    ...
    else
    {
        ...
        while (1) // The main loop of program
        {
            ...
            {
                // If something is gone in master socket => new client request to connect
                if ((new_socket = accept(master_socket, (const struct sockaddr *)(&client_addr), (socklen_t *)&client_addr)) < 0) {
                    perror("Error connecting");
                    continue;
                }
                else {
