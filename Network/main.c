    ...
    for (ifa = ifap; ifa != NULL; ifa = ifa->ifa_next) {
        if (ifa->ifa_addr->sa_family == AF_INET) {
            ...
            if (ifa->ifa_flags & IFF_UP && ifa->ifa_flags & IFF_RUNNING && ifa->ifa_addr->sa_family == AF_INET) {
                ...
                if (ipAddr == NULL) ipAddr = malloc(ip_addr_length+1);
                strncpy(ipAddr, ip_address, ip_addr_length);
                ipAddr[ip_addr_length] = '\0';
                freeifaddrs(ifap);
                return;
            }
        }
...
int main(int argc, char **argv) {
    ...
    if (mode == 1) {
        ...
        while (1) // The main loop of program
        {
            ...
            if (FD_ISSET(master_socket, &readfds)) {
                ...
                if ((new_socket = accept(master_socket, (struct sockaddr *)(&address), (socklen_t*)&addrlen)) < 0) {
                    ...
                    char *x = malloc(100);
                    sprintf(x, "current number is %d\n", myRoom.currentNumber);
                    write(1, x, strlen(x));
        ...
        if (val_recv < 0) {
            ...
            connectToRoomNetwork(myRoom, &clients);
        }
